        ...
        try {
            ...
            while( clienteEnTienda ) {
                ...
                while(condicionAddCarrito) {
                    ...
                    if( idArticulo.equals("fin") ) {
                        ...
                        if( carrito.getCarrito().size() < 1 ) {
                            ...
                            for(;;) {
                                ...
                                idArticulo = br.readLine();
                                seleccion = idArticulo.matches("\\d+") ? Integer.parseInt( idArticulo ) : -5;

                                if( idArticulo.equals("c") )
                                    break;
                                if( seleccion <= 0 || seleccion > carrito.getCarrito().size() ) {
                                    System.out.println("Seleccione un identificador de artíulo válido");
                                } else if( seleccion == -5 ) {
                                    System.out.println("Seleccione una opción correcta");
                                } else {

                                    Catalogo articulo = carrito.getCarrito().get( seleccion - 1 );
                                    int idCatalogo = -5;
                                    for( int i = 0; i < catalogo.size(); i++ ) {
                                        if( catalogo.get(i).getNombre().equals( articulo.getNombre() ) )
                                            idCatalogo = i;
                                        else
                                            break;
                                    } 
                                    ...
                                    for(;;) {
                                        ...
                                        System.out.println("Articulo: " + articulo.getNombre() + "\nTotal en carrito: " + articulo.getExistencias() );
                                        System.out.println(
                                            "\nSeleccione la cantidad que desea para el carrito." +
                                            "\nEscriba 'eliminar' para borrar el artículo del carrito." +
                                            "\nEscriba 'c' para cancelar."
                                        );
    
                                        String opcion = br.readLine();
                                        int cantNueva = opcion.matches("\\d+") ? Integer.parseInt(opcion) : -5;
    
                                        if( opcion.equals("c") )
                                            break;
                                        else if( opcion.equals("eliminar") ) {

                                            catalogo.get( idCatalogo ).setExistencias( catalogo.get( idCatalogo ).getExistencias() + articulo.getExistencias() );
                                            carrito.getCarrito().remove( seleccion - 1 );
                                            System.out.println("\nEl artículo fue eliminado del carrito");
                                            break;

                                        } else if( cantNueva <= 0 || cantNueva > catalogo.get( idCatalogo ).getExistencias() + articulo.getExistencias() )
                                            System.out.println("\nSeleccione una cantidad válida mayor a 0 y menor o igual a las existencias del artículo en el carrito");
                                        else {

                                            int diferencia = articulo.getExistencias() - cantNueva;
                                            catalogo.get( idCatalogo ).setExistencias( catalogo.get( idCatalogo ).getExistencias() + diferencia );
                                            carrito.getCarrito().get( seleccion - 1 ).setExistencias( cantNueva );
                                            System.out.println("\nLa cantidad de este artículo ha sido actualizada");
                                            totalArtículos -= diferencia;
                                            break;
                    ...
                    } else if( seleccion <= 0 || seleccion > catalogo.size() ) {
                        System.out.println("Seleccione un identificador de artíulo válido");
                        cantCorrecta = true;
                    } else if( seleccion == -5 ) {
                        System.out.println("Seleccione una opción correcta");
                ...
                if( clienteEnTienda && ( carrito.getCarrito().size() > 0 ) ) {
                    ...
                    Catalogo.guardar( catalogo, new File( "./catalogo.txt" ));
